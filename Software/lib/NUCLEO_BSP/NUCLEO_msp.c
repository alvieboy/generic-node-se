...
void HAL_UART_MspInit(UART_HandleTypeDef *uartHandle)
{
    ...
    if (uartHandle->Instance == DEBUG_USART)
    {
        ...
        NUCLEO_BSP_hdma_tx.Instance = DEBUG_USART_TX_DMA_CHANNEL;
        NUCLEO_BSP_hdma_tx.Init.Request = DEBUG_USART_TX_DMA_REQUEST;
        NUCLEO_BSP_hdma_tx.Init.Direction = DMA_MEMORY_TO_PERIPH;
        NUCLEO_BSP_hdma_tx.Init.PeriphInc = DMA_PINC_DISABLE;
        NUCLEO_BSP_hdma_tx.Init.MemInc = DMA_MINC_ENABLE;
        NUCLEO_BSP_hdma_tx.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
        NUCLEO_BSP_hdma_tx.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
        NUCLEO_BSP_hdma_tx.Init.Mode = DMA_NORMAL;
        NUCLEO_BSP_hdma_tx.Init.Priority = DMA_PRIORITY_LOW;

        if (HAL_DMA_Init(&NUCLEO_BSP_hdma_tx) != HAL_OK)
        {
            msp_error_handler();
        }

        if (HAL_DMA_ConfigChannelAttributes(&NUCLEO_BSP_hdma_tx, DMA_CHANNEL_NPRIV) != HAL_OK)
        {
            msp_error_handler();
        }

        /* Associate the initialized DMA handle to the UART handle */
        __HAL_LINKDMA(uartHandle, hdmatx, NUCLEO_BSP_hdma_tx);
...
void HAL_I2S_MspInit(I2S_HandleTypeDef *spiHandle)
{
    ...
    if (spiHandle->Instance == MICROPHONE_SPI)
    {
        ...
        NUCLEO_BSP_microphone_hdma_rx.Instance = MICROPHONE_RX_DMA_CHANNEL;
        NUCLEO_BSP_microphone_hdma_rx.Init.Request = MICROPHONE_RX_DMA_REQUEST;
        NUCLEO_BSP_microphone_hdma_rx.Init.Direction = DMA_PERIPH_TO_MEMORY;
        NUCLEO_BSP_microphone_hdma_rx.Init.PeriphInc = DMA_PINC_DISABLE;
        NUCLEO_BSP_microphone_hdma_rx.Init.MemInc = DMA_MINC_ENABLE;
        NUCLEO_BSP_microphone_hdma_rx.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
        NUCLEO_BSP_microphone_hdma_rx.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
        NUCLEO_BSP_microphone_hdma_rx.Init.Mode = DMA_CIRCULAR;
        NUCLEO_BSP_microphone_hdma_rx.Init.Priority = DMA_PRIORITY_LOW;

        if (HAL_DMA_Init(&NUCLEO_BSP_microphone_hdma_rx) != HAL_OK)
        {
            msp_error_handler();
        }

        if (HAL_DMA_ConfigChannelAttributes(&NUCLEO_BSP_microphone_hdma_rx, DMA_CHANNEL_NPRIV) != HAL_OK)
        {
            msp_error_handler();
        }

        /* Associate the initialized DMA handle to the SPI handle */
        __HAL_LINKDMA(spiHandle, hdmarx, NUCLEO_BSP_microphone_hdma_rx);
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *spiHandle)
{
    ...
    if (spiHandle->Instance == MICROPHONE_SPI)
    {
        ...
        NUCLEO_BSP_microphone_hdma_rx.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
        NUCLEO_BSP_microphone_hdma_rx.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
        NUCLEO_BSP_microphone_hdma_rx.Init.Mode = DMA_NORMAL;
        NUCLEO_BSP_microphone_hdma_rx.Init.Priority = DMA_PRIORITY_HIGH;
...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *timerHandle)
{
    ...
    if (timerHandle->Instance == BUZZER_TIMER)
    {
        ...
        HAL_NVIC_SetPriority(BUZZER_TIMER_IRQn, NUCLEO_BSP_BUZZER_TIMER_IT_PRIORITY, 0);
...
void HAL_SUBGHZ_MspInit(SUBGHZ_HandleTypeDef *subghzHandle)
{
    ...
    HAL_NVIC_SetPriority(SUBGHZ_Radio_IRQn, NUCLEO_BSP_SUBGHZ_RADIO_IT_PRIORITY, 0);
...
void HAL_RTC_MspInit(RTC_HandleTypeDef *rtcHandle)
{
    ...
    if (rtcHandle->Instance == RTC)
    {
        ...
        /* RTC interrupt Init */
        HAL_NVIC_SetPriority(TAMP_STAMP_LSECSS_SSRU_IRQn, NUCLEO_BSP_RTC_IT_PRIORITY, 0);
        HAL_NVIC_EnableIRQ(TAMP_STAMP_LSECSS_SSRU_IRQn);
        HAL_NVIC_SetPriority(RTC_Alarm_IRQn, NUCLEO_BSP_RTC_IT_PRIORITY, 0);
